...
pub(crate) fn write_media_segment(
    ...
    let capacity = moof
        .len()
        .checked_add(8)
        .and_then(|size| size.checked_add(usize::try_from(payload_len).ok()?))
        .ok_or_else(|| Error::InvalidMedia("fragment size does not fit in memory".to_owned()))?;
    let mut output = Vec::with_capacity(capacity);
    output.extend_from_slice(&moof);
    write_box_header(&mut output, payload_len + 8, *b"mdat")?;
    for sample in samples {
        let bytes = source.read_range(ByteRange::new(sample.offset, u64::from(sample.size)))?;
